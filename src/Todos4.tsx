...
export default function Todos4() {
    ...
    const supportedUnits = [ // The supported units of the API
        "meter",
        "kilometer",
        "centimeter",
        "millimeter",
    ...
    useEffect(() => { // Runs convert when num, fromUnit, or toUnit are changed
        convert();
    }, [num, fromUnit, toUnit]);

    const convert = () => { // Convert function
        ...
        fetch(url) // Fetch the following things from the url link
            ...
            .then((data) => { // ???
                ...
                if (!isNaN(value)) { // If value is not a number set result to value. IDK why
                    setResult(value);

                } else { // set result to not a number
                    setResult(NaN);
                }
            })
    ...
    return (
        ...
        >
            ...
            <div>
                ...
                >
                    {(supportedUnits).map((unit) => ( // Add all the supported units of the API into the dropdown menue item
                        <option
                ...
                <div
                    style={{ marginTop: "20px", fontSize: "18px" }}>Result:

                    <strong>
                        {num} {fromUnit} = {isNaN(result) ? "Invalid input" : result.toFixed(2) + " " + toUnit}
                    </strong>
                </div>
